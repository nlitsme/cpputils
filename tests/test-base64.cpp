#include "unittestframework.h"

#include "base64encoder.h"
#include "base64encoder.h"

#include <array>

TEST_CASE("base64") {
    enum {
        E=1,  // encode must succeed.
        NP=8, // encode without padding.
        D=2,  // decode must succeed.
        FD=4, // decode must fail.
    };
    std::vector<std::tuple<std::vector<uint8_t>, int, std::string>> testcases = {
        { { }, E|D, "" },
        { { }, E|D|NP, "" },
        { { }, D,   "\n" },
        { { }, D,   "\t" },
        { { }, FD,   "." },
        { { }, FD,   "A" },
        { { }, FD,   "A=" },
        { { }, FD,   "A==" },
        { { }, FD,   "A===" },
        { { }, FD,   "AAAAA" },
        { { }, FD,   "AAAAA=" },
        { { }, FD,   "AAAAA==" },
        { { }, FD,   "AAAAA===" },

        { { }, D,   "\t" },
        { { }, D,   "=" },
        { { }, D,   "==" },
        { { }, D,   "===" },

        // from rfc4648
        { { 0x66                          }, E|D, "Zg==" },
        { { 0x66,0x6f                     }, E|D, "Zm8=" },
        { { 0x66,0x6f,0x6f                }, E|D, "Zm9v" },
        { { 0x66,0x6f,0x6f,0x62           }, E|D, "Zm9vYg==" },
        { { 0x66,0x6f,0x6f,0x62,0x61      }, E|D, "Zm9vYmE=" },
        { { 0x66,0x6f,0x6f,0x62,0x61,0x72 }, E|D, "Zm9vYmFy" },

        // without padding
        { { 0x66                          }, E|D|NP, "Zg" },
        { { 0x66,0x6f                     }, E|D|NP, "Zm8" },
        { { 0x66,0x6f,0x6f                }, E|D|NP, "Zm9v" },
        { { 0x66,0x6f,0x6f,0x62           }, E|D|NP, "Zm9vYg" },
        { { 0x66,0x6f,0x6f,0x62,0x61      }, E|D|NP, "Zm9vYmE" },
        { { 0x66,0x6f,0x6f,0x62,0x61,0x72 }, E|D|NP, "Zm9vYmFy" },

        { { 0x00 }, E|D|NP,   "AA" },
        { { 0x00, 0x00 }, E|D|NP,   "AAA" },
        { { 0x00, 0x00, 0x00 }, E|D|NP,   "AAAA" },
        { { 0x00, 0x00, 0x00, 0x00 }, E|D|NP,   "AAAAAA" },
        { { 0x00, 0x00, 0x00, 0x00, 0x00 }, E|D|NP,   "AAAAAAA" },


        { { 0x00 }, E|D,   "AA==" },
        { { 0x00, 0x00 }, E|D,   "AAA=" },
        { { 0x00, 0x00, 0x00 }, E|D,   "AAAA" },
        { { 0x00, 0x00, 0x00, 0x00 }, E|D,   "AAAAAA==" },
        { { 0x00, 0x00, 0x00, 0x00, 0x00 }, E|D,   "AAAAAAA=" },
        { { 0x00, 0x00, 0x00, 0x00, 0x00 }, D,   "AAAAAAA=\n" },
        { { 0x00, 0x00, 0x00, 0x00, 0x00 }, D,   "AAAA\nAAA=\n" },
        { { 0x00, 0x00, 0x00, 0x00, 0x00 }, D,   "\tAAAA\nAAA=\n" },
        { { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 }, E|D,   "AAAAAAAA" },
        { { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 }, D,   "AAAA\nAAAA" },
        { { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 }, D,   "AAAA\nAAAA\n" },
        { { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 }, D,   "AAAA\nAAAA=\n" },
        { { 0xff, 0xff, 0xff, 0xff, 0xff }, D,   "//////8=\n" },
        { { 0xff, 0xff, 0xff, 0xff, 0xff }, D,   "////\n//8=\n" },
        { { 0xff, 0xff, 0xff, 0xff, 0xff }, D,   "\t////\n//8=\n" },
        { { 0xff, 0xff, 0xff, 0xff, 0xff, 0xff }, E|D,   "////////" },
        { { 0xff, 0xff, 0xff, 0xff, 0xff, 0xff }, D,   "////\n////" },
        { { 0xff, 0xff, 0xff, 0xff, 0xff, 0xff }, D,   "////\n////\n" },
        { { 0xff, 0xff, 0xff, 0xff, 0xff, 0xff }, D,   "////\n////=\n" },

        { { 0xff }, E|D,   "/w==" },
        { { 0xff, 0xff }, E|D,   "//8=" },
        { { 0xff, 0xff, 0xff }, E|D,   "////" },
        { { 0xff, 0xff, 0xff, 0xff }, E|D,   "/////w==" },
        { { 0xff, 0xff, 0xff, 0xff, 0xff }, E|D,   "//////8=" },
        { { 0xff, 0xff, 0xff, 0xff, 0xff, 0xff }, E|D,   "////////" },
{ { 0x00, 0x10, 0x83, 0x10, 0x51, 0x87, 0x20, 0x92, 0x8b, 0x30, 0xd3, 0x8f, 0x41, 0x14, 0x93, 0x51, 0x55, 0x97, 0x61, 0x96, 0x9b, 0x71, 0xd7, 0x9f, 0x82, 0x18, 0xa3, 0x92, 0x59, 0xa7, 0xa2, 0x9a, 0xab, 0xb2, 0xdb, 0xaf, 0xc3, 0x1c, 0xb3, 0xd3, 0x5d, 0xb7, 0xe3, 0x9e, 0xbb, 0xf3, 0xdf, 0xbf },
E|D, "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/" },
{ { 0xff, 0xef, 0x7c, 0xef, 0xae, 0x78, 0xdf, 0x6d, 0x74, 0xcf, 0x2c, 0x70, 0xbe, 0xeb, 0x6c, 0xae, 0xaa, 0x68, 0x9e, 0x69, 0x64, 0x8e, 0x28, 0x60, 0x7d, 0xe7, 0x5c, 0x6d, 0xa6, 0x58, 0x5d, 0x65, 0x54, 0x4d, 0x24, 0x50, 0x3c, 0xe3, 0x4c, 0x2c, 0xa2, 0x48, 0x1c, 0x61, 0x44, 0x0c, 0x20, 0x40 },
E|D, "/+9876543210zyxwvutsrqponmlkjihgfedcbaZYXWVUTSRQPONMLKJIHGFEDCBA" },
{ {  },
E|D, "" },
{ {  },
E|D, "" },

        { { 0xff }, E|D|NP,   "/w" },
        { { 0xff, 0xff }, E|D|NP,   "//8" },
        { { 0xff, 0xff, 0xff }, E|D|NP,   "////" },
        { { 0xff, 0xff, 0xff, 0xff }, E|D|NP,   "/////w" },
        { { 0xff, 0xff, 0xff, 0xff, 0xff }, E|D|NP,   "//////8" },

    };
    SECTION("cases") {
        for (auto& [data, flags, txt] : testcases) {
            if (flags&E) {
                if (flags&NP) {
                    auto e = base64_encode_unpadded(data);
                    CHECK(e == txt);
                }
                else {
                    auto e = base64_encode(data);
                    CHECK(e == txt);
                }
            }
            if (flags&D) {
                auto d = base64_decode(txt);
                CHECK(d == data);
            }
            if (flags&FD) {
                CHECK_THROWS(base64_decode(txt));
            }
        }
    }
    SECTION("types") {
        std::array<uint8_t, 3> a = { 0, 1, 2 };
        CHECK( base64_encode(a) == "AAEC" );

        std::vector<uint8_t> v = { 0, 1, 2 };
        CHECK( base64_encode(v) == "AAEC" );

        std::basic_string<uint8_t> bs = { 0, 1, 2 };
        CHECK( base64_encode(bs) == "AAEC" );
#if  __cplusplus >= 201703L
        CHECK( base64_encode((std::basic_string_view<uint8_t>)bs ) == "AAEC" );
#endif

        std::vector<int> iv = { 0, 1, 2 };
        CHECK( base64_encode(iv) == "AAEC" );

        std::string txt = "aaaa";
        CHECK( base64_decode(txt) == std::vector<uint8_t>{0x69, 0xa6, 0x9a} );
#if  __cplusplus >= 201703L
        CHECK( base64_decode((std::string_view)txt) == std::vector<uint8_t>{0x69, 0xa6, 0x9a} );
#endif

        std::basic_string<int> itxt = { 'a', 'a', 'a', 'a' };
        CHECK( base64_decode(itxt) == std::vector<uint8_t>{0x69, 0xa6, 0x9a} );
#if  __cplusplus >= 201703L
        CHECK( base64_decode((std::basic_string_view<int>)itxt) == std::vector<uint8_t>{0x69, 0xa6, 0x9a} );
#endif

        std::basic_string<uint8_t> btxt = { 'a', 'a', 'a', 'a' };
        CHECK( base64_decode(btxt) == std::vector<uint8_t>{0x69, 0xa6, 0x9a} );
#if  __cplusplus >= 201703L
        CHECK( base64_decode((std::basic_string_view<uint8_t>)btxt) == std::vector<uint8_t>{0x69, 0xa6, 0x9a} );
#endif
    }
    SECTION("invalid") {
        std::string txt = "x";

        for (int i = 0 ; i < 256 ; i++)
        {
            if (i == '\t' || i == '\n' || i == '\r' || i == 0xc || i == ' '
                    || (i>='0' && i<='9')
                    || (i>='a' && i<='z')
                    || (i>='A' && i<='Z')
                    || i=='=' || i=='/' || i=='+')
                continue;
            txt[0] = i;

            INFO("checking char " << i);
            CHECK_THROWS( base64_decode(txt) );
        }
    }
}
